...
boundary points of the same objective. In controlled online experiments across two environment
families, a noise-valuation control establishes that the value signal itself, not the routing
machinery, drives performance under capacity pressure. Routing throughput obeys a dose--response law in
transition costs spanning $175\times$, while maintenance tariffs never bind at tested scales; the
operative price of memory is movement, not storage. Several pre-registered predictions failed, and we
report these failures with diagnoses: residence-time bimodality is structural rather than priced;
promotion timing is set by switching costs rather than prediction errors at every testable operating
point; and a port to a nonlinear predictor falsified our hypothesis that capacity pressure favors the
priced router, localizing credit misassignment through the nonlinearity as the binding obstacle. We
conclude that memory hierarchy structure can be derived rather than designed, and that trustworthy
valuation under nonlinearity is the central open problem this framing exposes.
...
cost of holding it.
\item \textbf{Controlled evidence.} Across two environment families and four nonstationarity regimes
with policy-matched baselines ($n=10$ per cell), noise valuation---identical machinery with the value
signal replaced by noise---is consistently the worst or tied-worst policy, establishing that predictive
value, not sparsity or bookkeeping, carries performance under capacity pressure.
...
$M_s{=}15$), and value EMA; they differ only in routing policy: PES; random-routing (identical
machinery, noise valuation); clock-value and clock-magnitude (fixed-period swaps); and single-store
decay. Dense SGD is reported as an upper reference. Each cell uses $n=10$ seeds; comparisons use paired
...
Under the corrected margin router, no policy dominates across regimes: fixed-clock consolidation is
significantly better in Bernoulli-stationary retention ($p=0.021$), while drift and volatile regimes
show no significant separation among valued policies. We therefore do not claim per-regime superiority
for priced routing at this scale; the claim that survives replication is value \emph{necessity}, not
rule \emph{sufficiency} --- consistent with the price-trust analysis of Section~\ref{sec:nnport}.
...
corrected margin router). Fixed-clock consolidation leads stationary retention; valued policies
tie under drift; noise valuation is worst or tied-worst throughout.}
\label{fig:main}
\end{figure}
...
The program yields one positive design principle and one negative boundary, and we regard the pair as
the contribution. Positively: persistence economics is a coherent normative frame. It derives routing
from a single priced objective, recovers established mechanisms as corners of its tariff space
(Table~\ref{tab:corners}), provably allocates optimally given prices (Propositions~\ref{prop:static},
\ref{prop:dynamic}), and exposes movement cost---not storage cost---as the operative control knob,
with a $175\times$ dose--response law and three behavioral regimes. Negatively: the cheapest
instantiation of price discovery does not clear the bar that the frame itself sets. Under capacity
pressure on a nonlinear substrate, first-order gradient prices misattribute credit often enough that an
inflexible clock beats optimal-in-theory allocation with untrustworthy prices. The two results are two
sides of one statement: \emph{allocation is solved; price discovery is not}.
...
allocation framework the answer divides cleanly: derivation is possible in principle---one objective
recovers known mechanisms as corner pricings and allocates provably optimally given prices---and
incomplete in practice, because naive price discovery through a nonlineary predictor cannot yet be
trusted exactly where pricing matters most, under pressure. The framework organizes existing mechanisms
onto a single tariff landscape, contributes measurable laws about what governs routed memory, and
identifies trustworthy valuation as the concrete open problem standing between this formulation and
competitive learned memory hierarchies.
